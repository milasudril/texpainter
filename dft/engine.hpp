...
namespace Texpainter::Dft
{
	...
	public:
		...
		{
			auto i = std::ranges::find_if(m_plans, [size](auto const& val) { return val.m_size == size; });
			...
			if constexpr(dir == Direction::Forward) { i->m_plan_fwd.execute(input_buffer, output_buffer); }
			else
			{
