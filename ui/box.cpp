...
public:
	...
	void _add(GtkWidget* handle) noexcept
	{
		...
		{
			gtk_box_pack_end(m_handle, handle, m_mode.flags & Expand, m_mode.flags & Fill, m_mode.padding);
		}
		else
		{
			gtk_box_pack_start(m_handle, handle, m_mode.flags & Expand, m_mode.flags & Fill, m_mode.padding);
		}
	}
...
void* Texpainter::Ui::Box::toplevel() const
{
