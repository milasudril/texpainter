...
namespace Texpainter
{
	...
	public:
		    ...
		    , m_img_view{m_root.insertMode(Ui::Box::InsertMode{0, Ui::Box::Expand | Ui::Box::Fill})}
		{
			...
			m_seg_length.inputField().eventHandler<ControlId::SegLength>(*this).value(
			    Ui::SliderValue{std::log2((m_generator.segLength() + 1.0) / 7.0 + 1.0)});
			m_branch_prob.inputField().eventHandler<ControlId::BranchProb>(*this).value(
			    Ui::SliderValue{m_generator.branchProb() / (0.5 - 1.0 / 256.0)});
			m_max_length.inputField().eventHandler<ControlId::MaxLength>(*this).value(
			    Ui::SliderValue{m_generator.maxLength()});
		}
	...
	{
		auto const val = static_cast<int>(1.0 + slider.value().value() * 7.0);
	...
	{
		auto const val = static_cast<float>(std::exp2(-10.0 * (1.0 - slider.value().value())));
		m_preview      = m_generator.lineWidth(val)(m_preview.size());
		m_img_view.image(m_preview);
	}

	template<>
	inline void CrackCreator::onChanged<CrackCreator::ControlId::LineWidthGrowth>(
	    Ui::Slider& slider)
	{
		auto const val = slider.value().value();
		m_preview      = m_generator.lineWidthGrowth(val)(m_preview.size());
		m_img_view.image(m_preview);
	}

	template<>
	inline void CrackCreator::onChanged<CrackCreator::ControlId::NoiseModulation>(
	    Ui::Slider& slider)
	{
		auto const val = slider.value().value();
	...
	{
		auto const val = static_cast<float>(std::exp2(-7.0 * (1.0 - slider.value().value()) - 1.0));
	...
	{
		auto const val = (0.5 - 1.0 / 256.0) * slider.value().value();
