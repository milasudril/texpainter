	...
	public:
		constexpr explicit DiscretePdf(std::array<ProbType, N> const& vals):m_pdf{}
		{
			...
			if(i_begin == std::end(vals)) [[unlikely]]
			{
				...
				std::transform(std::begin(m_pdf), std::end(m_pdf), std::begin(m_pdf), [](auto val){return val / N;});
				m_first_nonzero = 0;
				m_last_nonzero = N - 1;
				return;
			}
			...
			std::transform(std::begin(m_pdf),
			               std::end(m_pdf),
			               std::begin(m_pdf),
			               [sum = m_pdf[m_last_nonzero]](auto val) {
							   return val / sum;
						});
		...
		{
			auto const i = std::ranges::upper_bound(m_pdf, random_value);
