...
namespace Texpainter
{
	...
	inline auto upperLimit(AxisAlignedBoundingBox box) { return box.origin() + box.radius(); }

	inline auto lowerLimit(AxisAlignedBoundingBox box) { return box.origin() - box.radius(); }

	inline auto size(AxisAlignedBoundingBox box) { return 2.0 * box.radius(); }

	inline AxisAlignedBoundingBox rotate(AxisAlignedBoundingBox box,
	                                     Angle ϴ,
	                                     vec2_t O = vec2_t{0.0, 0.0})
	{
		...
		auto const rot_x2 = vec2_t{cos(ϴ), sin(ϴ)};
		auto const rot_y2 = vec2_t{-sin(ϴ), cos(ϴ)};
		return AxisAlignedBoundingBox{transform((box.origin() - O), rot_x2, rot_y2) + O, radius};
	}
