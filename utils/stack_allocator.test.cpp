...
namespace Testcases
{
	void texpainterPreallocStackAllocatorFuzzer()
	{
		...
		{
			auto alloc_or_free =
			    std::bernoulli_distribution{static_cast<double>(pointers_in_use_size) / 256.0};
			...
			{
				auto which =
				    std::uniform_int_distribution{static_cast<size_t>(0), pointers_in_use_size - 1};
				...
				{
					std::shift_left(
					    std::begin(pointers_in_use) + index, std::end(pointers_in_use), 1);
				}
			}
