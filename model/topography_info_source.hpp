//@	{
//@	 "targets":[{"name":"topography_info_source.hpp", "type":"include"}]
//@	}

#ifndef TEXPAINTER_MODEL_TOPOGRAPHYINFOSOURCE_HPP
#define TEXPAINTER_MODEL_TOPOGRAPHYINFOSOURCE_HPP
...
namespace Texpainter::Model
{
	...
	public:
		...
		explicit TopographyInfoSource(std::string&& name): m_name{std::move(name)} {}
		...
		TopographyInfoSource& source(Span2d<FilterGraph::TopographyInfo const> src)
		{
			r_data = src;
			return *this;
		}
	...
	private:
		Span2d<FilterGraph::TopographyInfo const> r_data;
		std::string m_name;
	};
}

inline void Texpainter::Model::TopographyInfoSource::operator()(
    FilterGraph::ImgProcArg<InterfaceDescriptor> const& args) const
{
	...
	auto const src_ratio  = aspectRatio(r_data);
	auto const sink_ratio = aspectRatio(ret);
	auto const r          = src_ratio > sink_ratio
	                   ? static_cast<double>(ret.width()) / static_cast<double>(r_data.width())
	                   : static_cast<double>(ret.height()) / static_cast<double>(r_data.height());
	auto const scale_factor = 1 / r;

	auto const src = r_data;
	...
	for(int row = static_cast<int>(begin_coords[1]); row < static_cast<int>(end_coords[1]); ++row)
	{
		    ...
		    ++col)
		{
			   ...
			   && src_pos[1] < src.height())
			{
				...
				auto const z_00 = src(x_0, y_0).value();
				auto const z_01 = src(x_0, y_1).value();
				auto const z_10 = src(x_1, y_0).value();
				auto const z_11 = src(x_1, y_1).value();
