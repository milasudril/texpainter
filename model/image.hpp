...
namespace Texpainter::Model
{
	...
	{
		public:
			explicit Image(uint32_t width, DataBlock<Pixel>&& block): m_width{width}, m_block{std::move(block)}
			{}

			explicit Image(uint32_t width, uint32_t height, Pixel color_init = black()):m_width{width}, m_block{width*height}
